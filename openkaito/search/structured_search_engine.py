...
class StructuredSearchEngine:
    ...
    def vector_search(self, query):
        ...
        if query.index_name == "eth_denver":
            index_name = "eth_denver-base-v2-model"
        else :
            index_name = "eth_cc7-base-v2-model"
        embedding = text_embedding(query_string)
        ...
        # print(len(response))
        ranked_docsee = [doc["_source"] for doc in response["hits"]["hits"]]
        ...
        body = {
            "query": {
                "multi_match": {
                    "query": query_string,
                    "fields": [ "episode_title", "speaker", "text"],  # Replace with the actual fields you want to search
                    "type": "most_fields"  # You can also use "phrase" or "most_fields" depending on your needs
                }
            },
            "_source": {
                "excludes": ["embedding"],
        ...
        keyword = self.search_client.search(index=index_name, body=body)
        # print("size of response\n")
        # print(len(response))
        keyword_docs = [doc["_source"] for doc in keyword["hits"]["hits"]]
        for item in keyword_docs:
            flag = 0
            cur = item["doc_id"]
            for p in ranked_docsee:
                if cur == p["doc_id"]:
                    flag = 1
            if flag:
                continue
            ranked_docsee.append(item)

        # optional: you may implement yourselves additional post-processing filtering/ranking here
        return ranked_docsee
