...
class Evaluator:
    def __init__(self, llm_client, twitter_crawler=None) -> None:
        ...
        self.second_logger = logging.getLogger('second_logger')
        self.second_logger.setLevel(logging.INFO)
        # file_handler = logging.FileHandler('validator_output-dev.txt', mode='a')
        # file_handler = logging.FileHandler('val_discord.txt', mode='a')
        file_handler = logging.FileHandler('val_denver.txt', mode='a')
        file_handler.setLevel(logging.INFO)
        formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
        file_handler.setFormatter(formatter)
        self.second_logger.addHandler(file_handler)
    ...
    def evaluate_semantic_search(
        self, query: bt.Synapse, responses: list, dataset_dir: str, segments
    ):
        query_string = query.query_string
        size = query.size
        self.second_logger.info("\n\n\n\n")
        self.second_logger.info("start the evaluate semantic.")
        self.second_logger.info("query source is as follows.")
        self.second_logger.info(segments)
        self.second_logger.info("query is as follows.")
        self.second_logger.info(query)
        ...
        for i, response in enumerate(responses):
            ...
            try:
                ...
                llm_ranking_scores = self.llm_semantic_search_evaluation(
                    query_string, groundtruth_docs, k = 0
                )
                rank_scores[i] = ndcg_score(llm_ranking_scores, size)
        ...
        self.second_logger.info("rank_scores is as follows")
        self.second_logger.info(rank_scores)
        scores = rank_scores * uniqueness_scores
        self.second_logger.info("uniqueness_scores is as follows")
        self.second_logger.info(uniqueness_scores)
...
""",
    ...
    def llm_semantic_search_evaluation(self, query_string, docs, k, retries=3):
        ...
        try:
            ...
            if k == 0 :
                self.second_logger.info("Promt_docs as follows.\n")
                self.second_logger.info(prompt_docs)
            bt.logging.debug(
